...
const createBanner = () => {
  ...
  banner.innerHTML = `
    <div class="policyguard-shell">
      <div class="policyguard-brand">
        <div class="policyguard-badge">PG</div>
  ...
  style.textContent = `
    ...
    #${BANNER_ID} .policyguard-shell {
      display: grid;
      grid-template-columns: minmax(260px, 1.6fr) minmax(210px, .9fr) minmax(220px, 1fr) auto;
      gap: 16px;
      align-items: center;
      padding: 16px 18px;
  ...
  const setAnalysisState = (analysis) => {
    ...
    if (risksEl) {
      ...
      risksEl.innerHTML = topRisks.length > 0
        ...
        : '<span class="policyguard-chip">No major risks detected in quick scan</span>';
    }
